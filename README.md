...
- Day 15: added swipe-to-dismiss delete to the todo list — each item wraps in a
  `Dismissible` with a red delete background; `TodoProvider.deleteTodo` removes it
  from SQLite and reloads, while an undo snackbar calls `undoDelete` to restore it.
  Tested the provider logic and the swipe/undo widget flow with an in-memory DB.
